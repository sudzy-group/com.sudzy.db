...
let p = commander
  ...
  .option('-h, --remoteMySQLHost [value]', 'The remote MySQL Host argument')
	...
	.option('-s, --storeUser [value]', 'The store user')  
	.option('-c, --storePassword [value]', 'The store password')    
	.parse(process.argv);

if (!p.remotePouchDB || !p.remoteMySQLHost || !p.remoteMySQLUser ||!p.remoteMySQLDatabase || !p.storeUser || !p.storePassword) {
