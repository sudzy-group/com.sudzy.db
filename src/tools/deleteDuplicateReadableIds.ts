...
function safeDelete(id, callback) {
	orders.findIds('readable_id', id, { startsWith: true, limit: 40, descending: true})
	.then(os => {
		
		_.each(os, o => {
			console.log(o.id, o.value, parseInt(o.id.substr(0, 13), 10) < Date.now() - 24 * 60 * 60 * 1000 * 200)
			if (parseInt(o.id.substr(0, 13), 10) < Date.now() - 24 * 60 * 60 * 1000 * 200) {
				orders.get(o.id).then(order => {
					callback(order)
				})	
			}
		})
